import { articleSchema, type Article, type ArticleBlock, type ArticleSpan } from "./schemas/article.schema";
...
export function slugifyArticleText(value: string) {
  return value
    .toLowerCase()
    .split("")
...
export function parseArticleSpans(text: string): ArticleSpan[] {
...
function splitFrontmatter(source: string) {
  ...
  if (!match) {
    return { body: normalized, frontmatter: {} as Frontmatter };
  }
...
type SectionDraft = { blocks: ArticleBlock[]; heading: string; id: string };
type FaqDraft = { answer: string; question: string };
...
function parseArticleBody(body: string): BodyDraft {
  const draft: BodyDraft = { faq: [], intro: [], sections: [] };
  ...
  const pushBlock = (block: ArticleBlock) => {
  ...
  for (const rawLine of lines) {
    ...
    if (unordered || ordered) {
      ...
      listItems.push((unordered ? unordered[1] : (ordered as RegExpExecArray)[1]).trim());
      continue;
    }
...
function countSpanWords(spans: ArticleSpan[]) {
